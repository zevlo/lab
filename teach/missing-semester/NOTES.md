...
- Tidy CSV as a plot, gnuplot `using 1:2` + headless `dumb`/`png`, doubling spikes vs mean, periodic latency vs mean, and matplotlib faceting all landed. No friction reported; assume cold rebuilds done.
- A mean is now a known non-proof of “looks fine.” Split before you average. ggplot2 stays named.
- Next when asked: user said ready without picking; took lecture order → L0038 CPU profilers.

## L0038 design notes (MS Lecture 4 — CPU profilers, eighth slice)
- **Topic choice:** user gave no preference ("ready for next lesson"); took next in MS lecture order after visualizing — “CPU Profilers,” as the L0037 ask box named it first.
- **Primary sources:** MS Debugging and Profiling — “CPU Profilers” + profiling exercises (`perf stat`; `slow.c` + `perf record -g` / `perf report` / flame graph). `perf-stat(1)` / `perf-record(1)` (`-g` = call-graph; default unwind `fp`). Valgrind Callgrind manual (`--tool=callgrind`, `callgrind_annotate`, inclusive vs self). Brendan Gregg flame graphs (x = sample population, **not** a timeline; y = stack depth; width ∝ samples).
- **Scope:** sampling vs tracing; sampling preferred in production (lower overhead). `perf` as the Linux sampler: `perf stat` (software events live; hardware `cycles`/`instructions`/`branches` → `<not supported>` in Docker-on-Mac, same PMU gap as `rr`). `perf record -e cpu-clock -g` (software event — Gregg’s VM workaround) + `perf report`. Flame graph anatomy + MS pipeline (`perf script | stackcollapse-perf.pl | flamegraph.pl`). callgrind as the tracer: exact counts, caller/callee; shrink MS `slow.c` loops so Valgrind finishes. Speedscope / Perfetto / kcachegrind named. Language-specific (cProfile, py-spy, pprof, cargo-flamegraph) named.
- **Reuse:** L0036 `time` as “three numbers, no function names”; L0035 `-g` / `-fno-omit-frame-pointer` (perf’s default `fp` unwind; gcc `-O2` otherwise omits them — `perf-record(1)`). L0034 Valgrind as a supervised VM; L0030 `--privileged` is new (perf_event); L0037 bind-mount for the SVG.
- **Environment:** no `perf` on the Mac. Drill box: `debian` + `--privileged` + `linux-perf valgrind gcc git ca-certificates`, mount `/tmp/ms-cpu:/work`. Docker daemon was down on the agent host at write time; lesson still uses the corpus’s container posture. Hardware PMU will fail; software `cpu-clock` / `task-clock` should work. Callgrind on Apple-Silicon arm64 uses heuristics (Valgrind: “works best on x86/amd64”) — Ir-per-function still usable on this tiny C program. `/usr/bin/perf` is often a kernel-version wrapper — fall back to the versioned binary if the wrapper complains.
