...
**Next up:** L0033 in flight — MS lecture 4, third slice: network debugging (`tcpdump`; observation-point ladder curl→ss→strace→packets; MS invocations `port 80` / `-w capture.pcap` plus `-n`/`-c`/`-r`/`-A`; libpcap filters `host`/`port`/`src`/`dst` + and/or/not; reading a TCP line — `S`/`S.`/`.` handshake, `P.`/`F.`/`R.`; refused-RST vs dropped-silence triage with `ss -tlnp` cross-check; MS HTTPS/mitmproxy caveat; container drill with Mac-side curl; Wireshark/mitmproxy named only). L0032 done (system-call tracing). L0031 done (debugging fundamentals). Remaining Lecture-4 slices after L0033: ASan/Valgrind, rr, AI-for-debugging, and the Profiling half (`time`/`htop`/`perf`/`hyperfine`).
