...
**Next up:** L0034 in flight — MS lecture 4, fourth slice: memory debugging (MS exercise `uaf.c` as the drill spine, L0030 container posture with `gcc` + `valgrind` + `vim`; ASan `gcc -fsanitize=address -g` — report read as three connected stacks allocate→free→bad-access; sanitizer family TSan/MSan/UBSan as a map, not hands-on; Valgrind `--leak-check=full` for the no-recompile case with leak verdicts definitely-lost / still-reachable; callgrind/massif deferred to the Profiling half; `rr` named-only — MS: needs Linux hardware perf counters, fails in VMs, which includes the Docker-on-Mac box; `bpftrace` named-only — kernel-wide eBPF, root). L0033 done (network debugging: tcpdump; curl→ss→strace→packets ladder; refused-RST vs dropped-silence). L0031/L0032 done. Remaining Lecture-4 slices after L0034: AI-for-debugging (LLM on cryptic errors and stack traces — consumes ASan/strace output; distinct from L0029 form factors) and the Profiling half (`time` real/user/sys, resource monitoring `htop`/`free`/`lsof`, visualizing performance data, `perf`/callgrind, massif, `hyperfine`).
